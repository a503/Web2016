...
class UploadHandler
{
    ...
    protected function get_upload_path($file_name = null, $version = null) {
		...
		//return $this->options['upload_dir'].$this->get_user_path().$_REQUEST['ID'].'.'.strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
		...
		if (empty($file->error)) {
			$link =  mysql_pconnect( 'localhost', 'root', 'reading'  ) or
			die( 'Could not open connection to server' );
		
			mysql_select_db( 'rc', $link ) or 
				die( 'Could not select database rc' );
			
			mysql_query('set names utf8');

            $sQuery = 'UPDATE `std` SET `photo` = "'.$file->name.'" WHERE `no` = '.$_REQUEST['ID'];
