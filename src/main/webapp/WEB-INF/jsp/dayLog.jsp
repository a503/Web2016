...
<body>
	...
	<div id="wrapper"><!-- /#wrapper -->
		...
		<div id="page-wrapper">
			<div class="hidden-print row">
                <div class="col-lg-12">
                    <h1 class="page-header"><button class='hidden-print btn btn-info' id='print'>인쇄</button><span class='col-lg-11'>일일 업무일지</span></h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
							<div class="row">
								<div class="col-md-4 my-panel"">일일 업무일지 <span id="selector"></span></div>
								<div class="col-md-1 col-md-offset-7">
									<a type="button" class="btn btn-info btn-xs hidden-print"  id="btnNewDayLog">
										<i class="glyphicon glyphicon-plus"></i> 업무일지 작성
									</a>
								</div>
							</div>
                        </div>
            			<!-- Modal -->
                        <div class="modal fade" id="dayLogModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                        	<div class="modal-dialog">
                        		<div class="modal-content">
                        			<form action="dayLogEnroll.do" method="post">
                        				...
                        				<div class="modal-body">
                        					<fieldset>      
		                                        <div class="form-group">
											        ...
											        <input type="text" class="form-control" name="dlwriter" id="dlwriter" />
											    </div>
											    <div class="form-group">
											    ...
											    <div class="form-group">
											        ...
											        <input type="text" class="form-control" name="dllday" id="dllday"/>
											    </div>
											    <div class="form-group">
											        <label class="col-md-3 control-label">수업시간</label>
											        <input type="text" class="form-control" name="dlltime" id="dlltime"/>
											    </div>
											    <div class="form-group">
											        <label class="col-md-3 control-label">결석</label>
											        <input type="text" class="form-control" name="dlabsence" id="dlabsence"/>
											    </div>
											    <div class="form-group">
											        <label class="col-md-3 control-label">Comment</label>
											        <textarea class="form-control" name="dlcomment" id="dlcomment" rows="10"></textarea>
											    </div>		                                       
		                                    </fieldset>
                        				</div>
                                ...
                                <table class="table board_list table-condensed table-striped table-bordered table-hover text-center" id="dataTables-member" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center"><i class="glyphicon glyphicon-tags"></i> 등록일</th>
                                            <th class="text-center"><i class="glyphicon glyphicon-usd"></i> 수업명</th>
											<th class="text-center"><i class="glyphicon glyphicon-usd"></i> 수업일</th>
											<th class="text-center"><i class="glyphicon glyphicon-list"></i> 수업시간</th>
											<th class="text-center"><i class="glyphicon glyphicon-list"></i> 결석</th>
											<th class="text-center"><i class="glyphicon glyphicon-list"></i> Comment</th>                                     
                                        </tr>
                                    </thead>
									<tbody>
										<c:choose>
											<c:when test="${fn:length(list) > 0}">
							                    <c:forEach var="row" items="${list}" varStatus="status">
							                        <tr>
							                        	<input type="hidden" id="${row.DL_IDX }" name="SID" value="${row.DL_IDX }">
							                            <td>${row.DL_WRITER }</td>
							                            <td>${row.DL_DATE }</td>
							                            <td>${row.DL_L_NAME }</td>
							                            <td>${row.DL_L_DAY }</td>
							                            <td>${row.DL_L_TIME }</td>
							                            <td>${row.DL_ABSENCE }</td>
							                            <td>${row.DL_COMMENT }</td>         					                           							                            							                        
							                        </tr>
							                    </c:forEach>
							                </c:when>
							                <c:otherwise>
	...
	<script>
		...
		$('#btnNewDayLog').click(function(){
			$modalTitle.text('업무일지 작성');
			$('#dayLogModal').modal();
		});
